...
list.forEach((rule, index) => {
    const [count, letter, pass] = rule.split(' ');
    const [min, max] = count.split('-');
    const re = new RegExp(letter[0], 'g');
    const matches = []
    while (re.exec(pass) !== null) {
        matches.push(re.lastIndex)
    }
    if ((matches.includes(Number(min)) || matches.includes(Number(max))) && !(matches.includes(Number(min)) && matches.includes(Number(max)))) {
        validCount++
    }
})
